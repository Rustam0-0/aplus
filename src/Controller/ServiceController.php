...
class ServiceController extends AbstractController
{
    ...
    public function new(Request $request, ServiceRepository $serviceRepository, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
                ...
                $service->setDateAdd(new \DateTime());

                $aMimeTypes = array("image/gif", "image/jpeg", "image/jpg",
                "image/png", "image/x-png", "image/tiff");
                $objfichier = $request->files->get('service');
                $fichier = $objfichier['picture'];
                if (!empty($fichier)) {
                    if (in_array($fichier->getClientmimeType(), $aMimeTypes)) {
                        if ($fichier->move('assets/images/SERVICE/',
                            $fichier->getClientOriginalName())) {
                            $service->setPicture($fichier->getClientOriginalName());
                        }
                    }
                }

            $entityManager->persist($service); //insertion
            $entityManager->flush(); //exécute la requête
            $this->addFlash('success', 'Service est ajouté');
